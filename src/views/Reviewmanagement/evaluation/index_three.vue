<template>
  <div class="EvaluationThree">
        ...
        >
          ...
          <el-form-item>
            <el-select
              ...
              size="mini"
              :placeholder="'当前级评审状态('+menu_three+')'"
              clearable
              @change="elSelectStatus"
            >
              <el-option
    ...
    >
      ...
      >
        ...
        >
          ...
          <el-table
            :data="materialData"
            :height="isFullscreen ? isFullscreenHeight - 230 : 400"
            border
            highlight-current-row
            size="mini"
            style="width: 100%"
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  icon="el-icon-view"
                  round
                  :style="{backgroundColor:( editRowIndex1===scope.$index?'#42b983':'#fff')}"
                  @click="editRow1(scope)"
                />
              </template>
            </el-table-column>
            <el-table-column
    ...
    <el-dialog
      v-adaptive
      :title="'上传编辑'+uploadForm.Title"
      :close-on-click-modal="false"
      :visible.sync="EidtFileDialog"
      :width="device === 'desktop' ? '50%' : '99%'"
      @opened="uploadOpened"
      @close="uploadCancel"
    >
...
export default {
  ...
  components: { tinymce, period, cascaderFilter, filePreview, articlegroup },
  mixins: [tableHeight],
  data() {
    return {
      FileStatus: [ // 资料上传状态
  ...
  methods: {
      ...
      this.$refs.form3.validate(async v => {
        if (v) {
          ...
          this.MaterialReaultForm.Grade = Array.isArray(this.MaterialReaultForm.Grade)
            ? (this.MaterialReaultForm.Grade = this.Grades) : this.MaterialReaultForm.Grade;
          // 如果没选中等级，数值直接为空
    ...
    async SelectEvaluation() {
      ...
      this.EvaluationForm.GroupID = this.EvaluationForm.GroupIDArr.toString();
    ...
    async editRow1(scope, type) {
      try {
        const { data } = await SelectArticleDetail({
          ArticleID: scope.row.ArticleID
        });
        this.previewData = [];
        this.fileTitle = scope.row.CatalogCode;
        this.editRowIndex1 = scope.$index;
        this.editRowIndex2 = -1;
        this.ReviewIndex2 = -1;
        this.previewData = [...Preview(scope.row.Content), ...data];
        if (type === '上传编辑') {
          this.uploadForm = scope.row;
          this.uploadForm.File_list = this.previewData;
          this.$nextTick(() => {
            this.$refs.tinymceRef.setContent(scope.row.Content);
          });
          this.EidtFileDialog = true;
        } else {
    ...
    async editRow2(scope, type) {
      try {
        ...
        this.previewData = [...Preview(scope.row.Content), ...data];
        if (type === '上传编辑') {
          this.editRowIndex2 = scope.$index;
          this.uploadForm = scope.row;
          this.uploadForm.File_list = this.previewData;
    ...
    changeGroupID(val) {
      this.EvaluationForm.GroupID = val.toString();
      this.clickSelectEvaluation();
    },
    UnReviewChange(val) {
      this.clickSelectEvaluation();
