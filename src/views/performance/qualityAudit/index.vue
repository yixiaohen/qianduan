<template>
  <div class="EvaluationTwo">
    <el-card class="box-card">
      <div slot="header" :style="{ height: clearfixHeight + 'px' }" class="clearfix">
        <el-form :inline="true" :model="EvaluationForm" size="mini">
          ...
          <!-- <el-form-item>
            <el-select v-model="EvaluationForm.GroupID" size="mini" clearable style="width: 120px" placeholder="组别">
              <el-option v-for="GroupItem in SelectGroupListData" :key="GroupItem.GroupID" :label="GroupItem.GroupName" :value="GroupItem.GroupID" />
            </el-select>
          </el-form-item> -->
          <el-form-item>
            <el-input
        ...
        <el-tab-pane label="材料审核" name="first">
          <el-card style="height: 450px">
            ...
            >
              ...
              <el-table-column label="资料上传" fixed="right" align="center">
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    icon="el-icon-upload"
                    ...
                    "
                    @click="uploadViews(scope.row)"
                  />
                </template>
              </el-table-column>
              <el-table-column label="上传编辑" fixed="right" align="center">
                <template slot-scope="scope">
                  <el-button
                    size="mini"
                    icon="el-icon-upload"
            ...
            >
              <el-form-item label="标准">
                <el-input v-model="MaterialReaultForm.CatalogName" disabled>
                  <template slot="prepend">{{ MaterialReaultForm.CatalogCode }}</template>
                </el-input>
              </el-form-item>
      ...
      <el-form size="mini" label-width="80px">
        <el-form-item label="条款">
          <el-input v-model="seeStatusViewForm.CatalogName" placeholder="条款" disabled>
            <template slot="prepend">{{ seeStatusViewForm.CatalogCode }}</template>
          </el-input>
        </el-form-item>
    ...
    >
      ...
      >
        <div style="display: flex; flex-wrap: wrap">
          <el-form-item label="资料标题" prop="Title">
            <el-input v-model="uploadForm.Title" style="width: 200px" />
          </el-form-item>
          <el-form-item label="资料分类" prop="GroupID">
            <articlegroup
              style="width: 140px"
        ...
        <el-form-item label="版本号">
          <el-input v-model="uploadForm.VersionNumber" style="width: 200px" />
        </el-form-item>
        <el-form-item label="附件上传">
          <el-upload
            ...
            :action="IP"
          >
            <el-button size="mini" type="primary" :disabled="uploadForm.GroupID === ''">
              选择文件
            </el-button>
          </el-upload>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" @click="uploadCancel()">取 消</el-button>
        <el-button
          type="primary"
          size="mini"
          :loading="uploadEnd"
...
import { GetReviewArticleAudit, InsertArticle, SelectArticle, SelectArticleAudit, UpdateArticle } from '@/api/Article';
import {
...
export default {
  name: 'EvaluationTwo',
  components: { period, wangeditor, articlegroup, cascaderFilter },
  mixins: [tableHeight],
  data() {
    return {
      gradeType,
  ...
  methods: {
    ...
    async UpdateArticle(uploadForm) {
      this.$refs[uploadForm].validate(async(v) => {
        if (v) {
          ...
          this.uploadForm.Content = `${hrefs.join('')} ` + `${this.uploadForm.Content}`;
          try {
    ...
    async InsertArticle(uploadForm) {
      this.$refs[uploadForm].validate(async(v) => {
        if (v) {
          ...
          this.uploadForm.CatalogID = this.nodeValue.CatalogID;
          const hrefs = [];
          this.url.Content.forEach((e, i) => {
            hrefs.push(
              `<div>附件${
    ...
    InsertUpdateEvaluationProblem(e) {
      ...
      this.$refs.form2.validate(async(v) => {
        if (v) {
          ...
          try {
            const { msg } = await InsertUpdateEvaluationProblem(this.MaterialReaultForm);
            this.$message({
              type: 'success',
              message: msg
            });
    ...
    async UpdateRevoke(row) {
      const isAuditType = row.isAudit.includes(this.menu_two.replace('审核', ''));
      if (!isAuditType) {
        this.$message({
          type: 'warning',
          message: '未到当前级不可撤销！'
