  ...
  <div class="evaluation_not_validated">
    <split-pane split="horizontal">
        ...
        <div class="top-container">
          ...
          <div class="top-container-body">
            ...
            >
              ...
              <el-table-column prop="CatalogName" label="内容" min-width="300" />
              <el-table-column label="评审标准" width="70" align="center">
                <template slot-scope="{ row }">
                  <el-tag
                    size="mini"
                    effect="dark"
                    :type="row.IsCriterion ? 'success' : 'danger'"
                  >{{ row.IsCriterion ? "是" : "否" }}</el-tag>
                </template>
              </el-table-column>

              <el-table-column prop="Fraction" label="总分值" width="70" align="center" />
      ...
      <template slot="paneR">
        <div class="bottom-container">
          <el-tabs v-model="activeName" class="bottom-container">
            <el-tab-pane label="考评办法" name="first">
              <div class="bottom-container">
                ...
                >
                  ...
                  <el-table-column prop="ResortName" label="内容" min-width="300" />
            ...
            <el-tab-pane label="扣分原因" name="second">
              <div class="bottom-container">
                <div class="bottom-container-head">
                  <el-form size="mini" label-position="left" :inline="true">
                    <el-form-item>
                ...
                <el-table
                  ...
                  height="600"
                >
                  <el-table-column prop="PenaltiesContent" label="扣分内容" width="300" />
                  <el-table-column
    ...
    >
      <div class="content-books">
        <el-card class="box-card" shadow="never">
          <el-form
            ...
            size="mini"
          >
            <el-form-item label="手册标题" prop="ExpertID" class="el-select-books">
              <el-select
            ...
            <el-form-item label="扣分原因" prop="PenaltiesContent">
              <el-select
            ...
            >
              <el-form-item :label="'内容' + (index + 1)">
                <el-input v-model="book.Content" placeholder="请输入内容" clearable />
                <div style="display: flex; align-items: center; margin-top: 5px">
                  ...
                  />
                  <div v-for="items in book.Type" :key="items.key" class="el-slot">
                    <el-input
      ...
      <div class="footer">
        <span slot="footer" class="dialog-footer">
          <el-button size="mini" @click="addBooks">添 加</el-button>
          <el-button size="mini" @click="cancelExpertBooks">取 消</el-button>
          <el-button size="mini" type="primary" @click="submitExpert">确 定</el-button>
        </span>
      </div>
    </el-dialog>
    ...
    >
      ...
      >
        <el-form-item label="上级章节">
          <el-input v-model="rowObject.CatalogCode" disabled />
        </el-form-item>
        <el-form-item label="条款编号">
          <el-input v-model="installFormCatalog.CatalogCode" />
        </el-form-item>
        <el-form-item label="章节内容" prop="CatalogName">
          <el-input
        ...
        <el-form-item v-if="installFormCatalog.CatalogType !== 1" label="结果描述">
          <el-input
        ...
        <el-form-item label="标准类别">
          <el-select v-model="installFormCatalog.CatalogType" placeholder="请选择">
            <el-option
        ...
        <el-form-item
          ...
          prop="Fraction"
        >
          <el-input v-model="installFormCatalog.Fraction" clearable />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" @click="cancleAddCatalog">取 消</el-button>
        <el-button type="primary" size="mini" @click="doAddCatalog">确 定</el-button>
      </div>
    </el-dialog>
    <el-dialog
      title="评审标准修改"
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="updateFormCatalog.CatalogType !== 1" label="结果描述">
          <el-input
        ...
        <el-form-item label="标准类别">
          <el-select v-model="updateFormCatalog.CatalogType" placeholder="请选择">
            <el-option
              v-for="item in CatalogType"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item
          ...
          prop="Fraction"
        >
          <el-input v-model="updateFormCatalog.Fraction" clearable />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" @click="cancelUpdateCatalog">取 消</el-button>
        <el-button type="primary" size="mini" @click="doUpdateCatalog">确 定</el-button>
      </div>
    </el-dialog>

    <el-dialog :title="ResortTitle" :visible.sync="dialogFormVisible_resort">
      <el-form
        ref="restorData"
        :rules="rules"
        :model="ResortData"
      ...
      >
        <el-form-item label="评审要点">
          <el-input v-model="row_title" disabled />
        </el-form-item>
        <el-form-item label="内容" prop="ResortName">
          <el-input
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="mini" @click="dialogFormVisible_resort = false">取 消</el-button>
        <el-button
          type="primary"
          size="mini"
          @click="
    ...
    >
      <el-form :model="listQuery" size="mini">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="SelectPenalties" />
        </el-form-item>
      </el-form>
      <el-table
        ref="multipleTables"
    ...
    >
      ...
      >
        <el-form-item label="扣分原因">
          <el-input v-model="PenaltiesFrom.PenaltiesContent" type="textarea" />
        </el-form-item>
...
import { DeleteResort, InsertResort, SelectResort, UpdateResort } from '@/api/Resort';
import {
    ...
    UpdateResortContent() {
      this.$refs.restorData.validate(async(v) => {
        if (v) {
          ...
          try {
            const { msg } = await UpdateResort(form);
            this.$message({
              type: 'success',
              message: msg
            });
      ...
      this.$refs.updateFormCatalog.validate(async(v) => {
        if (v) {
          ...
          try {
            ...
            if (this.rowObject.ParentID) this.refreshRow(this.rowObject.ParentID);
            else this.refreshRoot();
          } catch (error) {
          } finally {
